...
```mermaid
graph TD
    ACCOUNTADMIN["ACCOUNTADMIN (Top-level role)"]
    SECURITYADMIN["SECURITYADMIN (Manages grants)"]
    SYSADMIN["SYSADMIN (Manages objects)"]
    USERADMIN["USERADMIN (Manages users/roles)"]
    CUSTOM_A["Custom Role A (e.g., DATA_ENGINEER)"]
    CUSTOM_B["Custom Role B (e.g., ANALYST)"]
    PUBLIC["PUBLIC (All users/roles)"]

...
### Scenario 1: Stale Stream
**Q:** A stream on a table has not been consumed for 15 days. The table has `DATA_RETENTION_TIME_IN_DAYS = 14`. What happens when you query the stream?
**A:** The stream is **stale** and returns an error. Since the offset has fallen outside the Time Travel retention window (14 days), the change data is no longer available. You must recreate the stream — the stale stream cannot be recovered.

### Scenario 2: Stream Staleness Period
**Q:** How is the staleness period for a stream calculated?
**A:** A stream becomes stale when it is not consumed within the source table's `DATA_RETENTION_TIME_IN_DAYS` period. For Standard streams, staleness = table's retention period (default 1 day, extendable to 14 days on Enterprise+ with the extended staleness feature, or up to 90 days matching Time Travel). If the stream offset exceeds this window, it becomes unreadable.

### Scenario 3: Task Tree Child Failure
**Q:** In a task DAG, the root task succeeds but a child task fails. Does the root task still run on the next schedule?
**A:** Yes — the root task runs independently on its schedule. The parent's success triggers children, but a child's failure does not prevent the parent from running again. The failed child will be skipped/retried based on task configuration, but the rest of the DAG continues.

### Scenario 4: MERGE with Duplicate Matches
**Q:** A MERGE statement matches multiple source rows to a single target row. What happens?
**A:** Snowflake raises a **non-deterministic MERGE error**. Unlike some databases that silently pick one matching row, Snowflake enforces determinism. You must deduplicate the source (using ROW_NUMBER or QUALIFY) or add conditions to ensure each target row matches at most one source row.

### Scenario 5: Clone and Streams
**Q:** You zero-copy clone a table that has a stream attached. Does the stream get cloned too?
**A:** No. Streams are **not** cloned with the table. The clone is a new table with no streams attached. You must explicitly create a new stream on the cloned table if you need CDC tracking on it.

### Scenario 6: Sequence in Multi-Row INSERT
**Q:** A sequence's `NEXTVAL` is used in a multi-row INSERT statement. Does each row get a unique value or the same value?
**A:** Each row gets a **unique** value. Within a single SQL statement, `NEXTVAL` advances once per row, generating distinct values for each inserted row.

### Scenario 7: Transient Table Cloned from Permanent
**Q:** You clone a permanent table into a transient table: `CREATE TRANSIENT TABLE t_clone CLONE perm_table`. What happens to Time Travel and Fail-Safe?
**A:** The transient clone inherits **transient properties**: maximum 0–1 day Time Travel and **no Fail-Safe**. The table type of the destination (transient) governs retention behavior, regardless of the source being permanent.

### Scenario 8: CREATE TABLE AS SELECT vs CLONE
**Q:** What are the key differences between `CREATE TABLE AS SELECT * FROM source` and `CREATE TABLE ... CLONE source`?
**A:** **CTAS** physically copies all data (new micro-partitions, immediate storage cost, no shared data with source). **CLONE** is metadata-only (zero additional storage at creation, shares micro-partitions with source, diverges only on modification). CLONE also preserves clustering keys, grants, and other table properties; CTAS creates a fresh table with default settings.

...
### Scenario 1: Remote Spilling in Query Profile
**Q:** The Query Profile shows "Bytes spilled to remote storage" for a large JOIN query. What is the recommended action?
**A:** Increase the warehouse size (scale up). Spilling to remote storage means intermediate results exceeded both memory and local SSD — a larger warehouse provides more memory per node, reducing or eliminating the spill.

### Scenario 2: Filter on Non-Clustering Column
**Q:** A table is clustered by `order_date`, but a query filters on `customer_id`. Can partition pruning still help?
**A:** Yes, if `customer_id` values happen to be naturally ordered (e.g., inserted sequentially over time) so that min/max metadata on micro-partitions can still eliminate ranges. However, pruning will be less effective than filtering on the clustering key itself.

### Scenario 3: Clustering Depth Interpretation
**Q:** `SYSTEM$CLUSTERING_INFORMATION` returns `average_depth = 1.0`. Is the table well-clustered?
**A:** Yes — a depth of 1.0 is perfect. It means each value range is contained within exactly one micro-partition with zero overlap, enabling maximum partition pruning.

### Scenario 4: Materialized View vs Summary Table
**Q:** A team needs a pre-aggregated summary of daily sales. Should they use a materialized view or manually create a summary table?
**A:** A materialized view is maintained automatically by Snowflake as base data changes. A manually created summary table requires user-managed refresh logic (tasks, procedures). If automatic maintenance is required, the materialized view is correct.

### Scenario 5: Search Optimization Service Query Types
**Q:** Which types of queries benefit from the Search Optimization Service?
**A:** Point lookups (equality `=`), `IN` lists, `LIKE` with non-leading wildcards, substring searches (`LIKE '%value%'`), geographic queries (GEOGRAPHY), and selective lookups on VARIANT paths. It does NOT help broad range scans or full table scans.

### Scenario 6: Slow Query at Peak Hours
**Q:** A query runs fine at midnight but is slow at 9 AM when many users are active. Is this a concurrency issue? Should you use a multi-cluster warehouse or a bigger warehouse?
**A:** This is a concurrency issue — queries are queuing because the warehouse is fully utilized. Use a **multi-cluster warehouse** (scale out) to handle more concurrent queries. Scaling up (bigger size) increases per-query memory but does not add concurrency slots.

### Scenario 7: EXPLAIN vs Query Profile
**Q:** When should you use EXPLAIN versus the Query Profile?
**A:** Use **EXPLAIN** before execution to validate the plan, check estimated partition pruning, and verify the optimizer's approach without consuming credits. Use **Query Profile** after execution to see actual runtime statistics including spilling, exact partitions scanned, time per operator, and actual data volumes.

### Scenario 8: 95% Partition Scan
**Q:** The Query Profile shows "Partitions scanned: 9,500 out of 10,000" (95%). What does this indicate?
**A:** Poor partition pruning — the query is scanning nearly the entire table. The filter columns are either not aligned with the table's physical organization, a function is applied to the filter column preventing pruning, or no effective clustering exists for those predicates. Consider adding a clustering key on the frequently filtered columns.

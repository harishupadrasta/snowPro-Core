...
```mermaid
graph TB
    VPS["Virtual Private Snowflake (VPS)<br/>Dedicated isolated environment<br/>Separate metadata store<br/>Dedicated virtual servers"]
    BC["Business Critical<br/>+ Database failover/failback<br/>+ Tri-Secret Secure<br/>+ HIPAA / PCI / FedRAMP<br/>+ Customer-managed keys<br/>+ Private connectivity (PrivateLink)"]
    ENT["Enterprise<br/>+ Multi-cluster warehouses<br/>+ Time Travel up to 90 days<br/>+ Materialized views<br/>+ Column-level security<br/>+ Dynamic data masking<br/>+ Search optimization service<br/>+ Row access policies<br/>+ Tag-based masking"]
    STD["Standard<br/>Complete SQL engine<br/>Data sharing<br/>External functions<br/>1-day Time Travel<br/>Continuous data protection<br/>Always-on encryption (AES-256)<br/>Secure data sharing"]
